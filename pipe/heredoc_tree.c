/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   heredoc_tree.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: soljeong <soljeong@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/23 10:11:48 by soljeong          #+#    #+#             */
/*   Updated: 2024/04/23 10:34:58 by soljeong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

static void	heredoc_tree(t_tree *tree, t_list *envp, \
	int *heredoc_count);

void	herdoc_tree_init(t_tree *tree, t_list *envp)
{
	int	heredoc_count;

	heredoc_count = 0;
	heredoc_tree(tree,envp,&heredoc_count);
}

static void	heredoc_tree(t_tree *tree, t_list *envp, \
	int *heredoc_count)
{
	char *filename;
	if (tree->left)
		heredoc_tree(tree->left, envp, heredoc_count);
	if (tree->token && tree->token->type == REDIRECT_HEREDOC)
	{
		filename = heredoc(tree->redirect->filename, heredoc_count, envp);
		tree->token->type = REDIRECT_IN;
		tree->redirect->filename = filename;
		(*heredoc_count)++;
	}
	if (tree->right)
		heredoc_tree(tree->right, envp, heredoc_count);
}